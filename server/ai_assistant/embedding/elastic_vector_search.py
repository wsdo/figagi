...
class ElasticVectorSearch(VectorStore, ABC):
    ...
    def add_texts(
        ...
        **kwargs: Any,
        ...
        for i, text in enumerate(texts):
            ...
            _id = str(uuid.uuid4())
            request = ElasticVectorSearch.generate_request(self.index_name, embeddings[i], text, metadata, _id)
            ids.append(_id)
    ...
    def from_texts(
        ...
        **kwargs: Any,
        ...
        for i, text in enumerate(texts):
            metadata = metadatas[i] if metadatas else {}
            request = ElasticVectorSearch.generate_request(index_name, embeddings[i], text, metadata) 
            requests.append(request)
    ...
    @staticmethod
    def generate_request(index_name, vector, text, metadata, _id = None): 
        ext_fields = {}
        if 'ext_fields' in metadata:
            ext_fields = metadata.get('ext_fields')
            metadata.pop('ext_fields')
